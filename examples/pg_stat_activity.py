...
class SparkmesActivity(Plugin):
    ...
    #   0      1         2         3      4          5             6
    Items = [
        ('calls_upd', 'pgsql.queries.sparkmes', 'number of queries (modify)', 'n/m',
         Plugin.DELTA.simple_change, 1, Plugin.VALUE_TYPE.numeric_unsigned),
        ('calls_sel', 'pgsql.queries.sparkmes', 'number of requests (select)', 'n/m',
         Plugin.DELTA.simple_change, 1, Plugin.VALUE_TYPE.numeric_unsigned),

        ('time_upd', 'pgsql.queries.sparkmes', 'time (modify)', Plugin.UNITS.ms,
         Plugin.DELTA.simple_change, 2, Plugin.VALUE_TYPE.numeric_float),
        ('time_sel', 'pgsql.queries.sparkmes', 'time (select)', Plugin.UNITS.ms,
         Plugin.DELTA.simple_change, 2, Plugin.VALUE_TYPE.numeric_float),

        ('callsfirm', 'pgsql.queries.sparkmes', 'number of requests (firm)', 'n/m',
         Plugin.DELTA.simple_change, 1, Plugin.VALUE_TYPE.numeric_unsigned),
        ('callsip_s', 'pgsql.queries.sparkmes', 'number of requests (ip)', 'n/m',
         Plugin.DELTA.simple_change, 1, Plugin.VALUE_TYPE.numeric_unsigned),

        ('timefirm', 'pgsql.queries.sparkmes', 'time (firm)', Plugin.UNITS.ms,
         Plugin.DELTA.simple_change, 2, Plugin.VALUE_TYPE.numeric_float),
        ('timeip_s', 'pgsql.queries.sparkmes', 'time (ip)', Plugin.UNITS.ms,
         Plugin.DELTA.simple_change, 2, Plugin.VALUE_TYPE.numeric_float),
    ]
    # name, type, ('graph item', color, yaxisside, drawtype)
    #   0     1         2.0        2.1     2.2       2.3
    Graphs = [
        ('sparkmes activity (calls)', 0, (('calls_upd', 'CCFFCC', 1, 5),
                                            ('calls_sel', 'CCFFFF', 1, 5),
                                            ('calls_upd_Yprev', '009900', 1, 2),
                                            ('calls_sel_Yprev', '009999', 1, 2),),),
        ('sparkmes activity (time)', 0, (('time_upd', 'CCFFCC', 1, 5),
                                           ('time_sel', 'CCFFFF', 1, 5),
                                           ('time_upd_Yprev', '009900', 1, 2),
                                           ('time_sel_Yprev', '009999', 1, 2),),),
        ('sparkmes search (calls)', 0, (('callsfirm', 'CCFFCC', 1, 5),
                                       ('callsip_s', 'CCFFFF', 0, 5),
                                       ('callsfirm_Yprev', '009900', 1, 2),
                                       ('callsip_s_Yprev', '009999', 0, 2),),),
        ('sparkmes search (time)', 0, (('timefirm', 'CCFFCC', 1, 5),
                                      ('timeip_s', 'CCFFFF', 0, 5),
            ...
            for row in result:
                if key.endswith(row[0]):
                    ...
                    self.Yprev[idx] = round(
                        self.Coef * float(val) + (1 - self.Coef) * self.Yprev[idx], 1)
                    break
                else:
                    continue
            else:
                if self.Yprev[idx] is not None:
                    # исходных данных нет, уменьшаем значение
                    self.Yprev[idx] = self.Yprev[idx] / 3
            if self.Yprev[idx] is None or self.Yprev[idx] is not None and self.Yprev[idx] <= 0:
                ...
                if val is not None:
                    if item[6] == Plugin.VALUE_TYPE.numeric_float:
                        zbx.send('{0}[{1}]'.format(
                            zbxkey, key), float(val), delta)
                        self.log.info('{0}[{1}] '.format(
                            zbxkey, key) + str(val))
                    else:
                        zbx.send('{0}[{1}]'.format(
                            zbxkey, key), int(val), delta)
                        self.log.info('{0}[{1}] '.format(
                            zbxkey, key) + str(val))
                    zbx.send('{0}[{1}_Yprev]'.format(zbxkey, key),
                             float(self.Yprev[idx]), delta)
                    self.log.info('{0}[{1}_Yprev] '.format(
                        zbxkey, key) + str(self.Yprev[idx]))
                # debug...
                # if key == 'calls_upd':
